...
function doAttack(attacker, defender, atk, aBuff, dBuff, log, G, switchGuardMult = 1, standbyGuardMult = 1) {
  ...
  if (dBuff.reflect) {
    ...
    const rMoldBreaker = defenderAbility?.id === 'mold-breaker' || defenderAbility?.id === 'true-damage' || defenderAbility?.id === 'shadow-tag-pierce';
    ...
    const rAbilityDmgMult = ((rIsBlazeBoostFamily && rLowHpSelf && rIsOwnType) ? 1.1
      ...
      : (defenderAbility?.id === 'technician' && atk.dmg <= 60) ? 1.1 : 1) * rTintedLensMult;
    ...
    const rShieldTerm = (dBuff.ignoreShield || atk.ignoreShield || defenderAbility?.id === 'crowned-sword-might' || defenderAbility?.id === 'shadow-tag-pierce') ? 0 : (attackerAbility?.id === 'shield-invert' ? -aBuff.shield : aBuff.shield);
  ...
  const moldBreaker = attackerAbility?.id === 'mold-breaker' || attackerAbility?.id === 'true-damage' || attackerAbility?.id === 'shadow-tag-pierce';
  ...
  const stabMult = isOwnType ? (isAdaptabilityMajor ? 1.4 : isAdaptability ? 1.2 : 1.1) : 1;
  ...
  const multiscaleMult = (!moldBreaker && defenderAbility?.id === 'multiscale' && wasFullHp) ? 0.1 : 1;
  // 厚脂肪（thick-fat-pure）／硬岩（solid-rock-flat）2026-08-14新增：固定減傷，不進defAbilityMult
  // 乘法鏈，改成跟rockFieldReduction同一套「乘法鏈算完後再扣」寫法
  const thickFatPureReduction = (!moldBreaker && defenderAbility?.id === 'thick-fat-pure') ? 50 : 0;
  const solidRockFlatReduction = (!moldBreaker && defenderAbility?.id === 'solid-rock-flat') ? 30 : 0;
  ...
  if (mult === 0) {
    ...
    const shieldTerm = (aBuff.ignoreShield || atk.ignoreShield || attackerAbility?.id === 'crowned-sword-might' || attackerAbility?.id === 'shadow-tag-pierce') ? 0 : (defenderAbility?.id === 'shield-invert' ? -dBuff.shield : dBuff.shield);
    ...
    if (attackerAbility?.id === 'guts' && attacker.status) log.push({ text: `${attacker.name} 的${attackerAbility.name}發動，攻擊威力提升！`, cls: 'super' });
...
function triggerOnEnterSrv(poke, role, G, log, isFieldEntry = true, suppressAbility = false) {
  ...
  if (poke.ability.id === 'shock-stadium-dodge') {
...
async function handleMessage(ws, msg) {
    ...
    if (type === 'attack') {
      ...
      if (atk.support) {
        ...
        if ((STADIUM_DOUBLE_ATTACK[G.activeStadium?.id] === atkTypeForDouble || isMultiStrike) && attacker.cur > 0) {
          const secondHitMult = isMultiStrike ? 0.2 : 0.4;
          const secondAtk = { ...atk, _secondHitMult: secondHitMult }; // 見doAttack內_secondHitMult的說明，不能只砍atk.dmg
          if (defender.cur > 0) {
