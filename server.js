...
function clonePoke(p) {
  return { ...p, attacks: p.attacks.map(a => ({...a})), cur: p.hp, status: null,
    megaEnergy: p.mega ? 0 : undefined, megaEvolved: p.mega ? false : undefined };
}
...
function doAttack(attacker, defender, atk, aBuff, dBuff, log, G, switchGuardMult = 1) {
  ...
  if (mult === 0) {
    ...
    if (damage > 0) {
      ...
      if (attacker.mega && !attacker.megaEvolved) attacker.megaEnergy = Math.min(20, attacker.megaEnergy + megaGain);
      if (defender.mega && !defender.megaEvolved) defender.megaEnergy = Math.min(20, defender.megaEnergy + megaGain);
    }
...
function handleMessage(ws, msg) {
    ...
    // Mega 進化：免費行動，不結束回合，也不消耗能量
    if (type === 'mega_evolve') {
      if (G.turn !== role || G.pendingKOSwitch) return;
      const attacker = G[`${role}Deck`][G[`${role}Idx`]];
      if (!attacker.mega || attacker.megaEvolved || attacker.megaEnergy < 20) return;
      ...
      const log = [{ text: `${attacker.name} Mega 進化了！`, cls: 'special' }];
