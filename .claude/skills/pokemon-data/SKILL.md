...
- Battle-instance cloning: single-player `clone()`, PvP `clonePoke()` — both do a shallow `{...p}` plus deep-copy `attacks` and add `cur` (current HP) / `status`. A shallow spread means `ability`/`mega` carry over automatically — don't need to special-case them when cloning. Both `clone()`/`clonePoke()` additionally initialize `megaEnergy:0`/`megaEvolved:false` **only if** `p.mega` exists (`undefined` otherwise, not `0`/`false` — a Pokémon with no Mega form should never show a Mega bar, and `poke.mega && !poke.megaEvolved` is the standard guard used everywhere a Mega UI element or check is gated).
